...
def validate(sess, model, x_test, y_test, batch_size):
    ...
    for i in range(int(x_test.shape[0] / batch_size)):
        accuracy = accuracy + batch_size * model.validate(
            sess,
    ...
    if x_test.shape[0] % batch_size != 0:
        accuracy = accuracy + batch_size * model.validate(
            sess, x_test[(i + 1) * batch_size :], y_test[(i + 1) * batch_size :]
        )
...
def validate_bagging(sess, model, x_test, y_test, batch_size, num_sets):
    ...
    for i in range(num_sets):
        ...
        val_accs.append(
            validate(sess, model, x_test[indicies], y_test[indicies], batch_size)
        )
...
def train(
    ...
    for i in range(epochs):
        ...
        for j in range(int(x_train.shape[0] / batch_size)):
            ...
            loss, loss_summary = model.fit_batch(
                sess, x_train[temp_indicies], y_train[temp_indicies]
            )
            if summary_writer:
            ...
            if j % train_validation == 0:
                train_acc, train_summary = model.train_validate(
                    sess, x_train[temp_indicies], y_train[temp_indicies]
                )
                if summary_writer:
...
def prediction(sess, model, x_test, y_test, train_lst, val_lst, batch_size=32):
    ...
    for i in range(int(X_val.shape[0] / batch_size)):
        preds[batch_size * i : batch_size * (i + 1)] = model.predict(
            sess, X_val[batch_size * i : batch_size * (i + 1)]
        )
        y_preds[batch_size * i : batch_size * (i + 1)] = np.argmax(
            Y_val[batch_size * i : batch_size * (i + 1)], axis=1
        ).astype("uint8")
    i = int(X_val.shape[0] / batch_size)
    preds[batch_size * i :] = model.predict(sess, X_val[batch_size * i :])
    y_preds[batch_size * i :] = np.argmax(Y_val[batch_size * i :], axis=1).astype(
        "uint8"
    )
