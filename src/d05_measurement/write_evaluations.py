...
def write_evaluations(folder, prediction_source):
    ...
    file_names = [fn.split('.')[0] for fn in os.listdir(f'/home/ubuntu/data/01_raw/{folder}')]
    ...
    merge_df = ground_truths_df.merge(calculations_df, on=cols, suffixes=['_gt', '_calc'])
    ...
    numeric_df['measurement_value_gt'] = pd.to_numeric(numeric_df['measurement_value_gt'])
    numeric_df['measurement_value_calc'] = pd.to_numeric(numeric_df['measurement_value_calc'])

    abs_diff_df = numeric_df[cols].copy()
    abs_diff_df['score_type'] = 'absolute_difference' 
    abs_diff_df['score_value'] = np.abs(numeric_df['measurement_value_gt']-numeric_df['measurement_value_calc'])
    abs_diff_df.head()

    rel_diff_df = abs_diff_df.copy()
    rel_diff_df['score_type'] = rel_diff_df['score_type'].str.replace('absolute', 'relative')
    ...
    acc_df['score_value'] = (rec_df['measurement_value_gt'] == rec_df['measurement_value_calc']).astype(int)
    ...
    fig, ax = plot_confusion_matrix(y_true, y_calc, classes=classes,
                               title='Confusion matrix, without normalization')

    fig.savefig(f'../results/cm_{folder}_{prediction_source}', bbox_inches='tight')
    fig.savefig(f'../results/cm_{folder}_{prediction_source}.pdf', format='pdf', bbox_inches='tight')

    fig, ax = plot_confusion_matrix(y_true, y_calc, classes=classes, normalize=True,
                               title='Normalized confusion matrix')

    fig.savefig(f'../results/norm_cm_{folder}_{prediction_source}', bbox_inches='tight')
    fig.savefig(f'../results/norm_cm_{folder}_{prediction_source}.pdf', format='pdf', bbox_inches='tight')
    ...
    measurements = ["Left ventricular diastolic volume, mL", "Left ventricular systolic volume, mL", "Left ventricular ejection fraction, %"]
    ...
    value_describes = [numeric_df[numeric_df['measurement_name'] == measurement_name]['measurement_value_gt'].describe().astype(int) for measurement_name in measurement_names]
    median_iqr_strs = [f"{describe['50%']} ({describe['25%']}-{describe['75%']})" for describe in value_describes]

    abs_diff_describes = [abs_diff_df[abs_diff_df['measurement_name'] == measurement_name]['score_value'].describe().astype(int) for measurement_name in measurement_names]
    rel_diff_describes = [rel_diff_df[rel_diff_df['measurement_name'] == measurement_name]['score_value'].describe().astype(int) for measurement_name in measurement_names]
    abs_dev_median_strs = [f"{abs_diff_describe['50%']} ({rel_diff_describe['50%']})" for abs_diff_describe, rel_diff_describe in zip(abs_diff_describes, rel_diff_describes)]

    df = pd.DataFrame.from_dict({'Measurements': measurements, 'Number': numbers, 'Median Value (IQR)': median_iqr_strs, 'Median Absolute Deviation (% of Manual)': abs_dev_median_strs})
    df.to_csv(f'../results/measurement_comparison_{folder}_{prediction_source}.csv', index=False)
