...
def agg_probabilities(if_exists='replace'):
    ...
    probabilities = (
        probabilities_frames.groupby(
            ["study_id", "file_name", "model_name", "date_run", "img_dir"]
        )
        .agg(agg_cols)
        .reset_index(drop=False)
    )
    probabilities.rename(
        columns={"probabilities_frame_id": "frame_count"}, inplace=True
    )

    io_classification.save_to_db(probabilities, "probabilities_instances", if_exists)
    logger.info(
        "Aggregated probabilities saved to table classification.probabilities_instances"
    )
...
def predict_views(if_exists='replace'):
    ...
    predictions = probabilities.drop(
        columns=["probabilities_instance_id", "frame_count"]
    ).set_index(["study_id", "file_name", "model_name", "date_run", "img_dir"])

    predictions["view23_pred"] = predictions.idxmax(axis=1).apply(
