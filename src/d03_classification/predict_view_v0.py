        ...
        if os.path.exists(os.path.join(out_directory, outrawfilename)):
            ...
            try:
                ds = pydicom.read_file(
                    os.path.join(out_directory, outrawfilename), force=True
                )
                ...
                if y > 10:
                    ...
                    for n in m:
                        ...
                        outfile = (
                            os.path.join(out_directory, filename) + str(n) + ".jpg"
                        )
                        cv2.imwrite(
                            outfile,
                ...
                print(
                    out_directory + "\t" + outrawfilename + "\t" + "error", counter, e
                )
        else:
            counter = counter + 1
            time.sleep(3)
        ...
        if filename.startswith(prefix):
            ...
            if ("NumberOfFrames" in dir(ds)) and (ds.NumberOfFrames > 1):
                ...
                command = (
                    ...
                    + " "
                    + os.path.join(out_directory, outrawfilename)
                )
...
def main():
    ...
    out = open(
        probabilities_dir + model + "_" + dicomdir_basename + "_probabilities.txt", "w"
    )
